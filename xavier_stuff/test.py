...
class Demo(Base):
    def __init__(self):
        ...
        #suffix = self.input_video_path.split("/")[-2]
        #print(suffix)
    ...
    def process_video(self, video_file_path=None):
        ...
        for frame_id in range(video_length):
            ...
            # render the output on a frame and append result
            vis_eval_results = self.visualizer.visualize_side_result(outputs['verts'], 
                                                                     outputs['verts_camed'], 
                                                                     vis_dict, 
                                                                     reorganize_idx=outputs['reorganize_idx'].cpu().numpy())
            result_frames_side.append(vis_eval_results[0])

            # - Render the front view without concatening the original

            # -- Original code
            # render the output on a frame and append result
            vis_eval_results = self.visualizer.visulize_result_onorg(outputs['verts'], 
                                                                     outputs['verts_camed'], 
                                                                     vis_dict, 
                                                                     reorganize_idx=outputs['reorganize_idx'].cpu().numpy())
            #print("image.shape in test.py:", vis_eval_results[0].shape)
        ...
        if self.save_video_results:
            ...
            video_save_name_side = os.path.join(self.output_dir, video_basename+'_results_side.mp4')
            
            print('Writing results to {}'.format(video_save_name))
            
            # convert rendered frames to a video
            frames2video(result_frames, video_save_name, fps=args.fps_save)
            frames2video(result_frames_side, video_save_name_side, fps=args.fps_save)
    ...
    def webcam_run_local(self, video_file_path=None):
        ...
        for i in range(10):
            ...
            self.single_image_forward(np.zeros((256,256,3)).astype(np.uint8))
        counter = Time_counter(thresh=1)
        while True:
