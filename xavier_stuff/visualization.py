...
class Visualizer(object):
    ...
    def visualize_renderer(self, verts, images=None, reorganize_idx=None, angle=None, axis=None, thresh=0.2,visible_weight=0.9, scale_thresh=100):
        verts = verts.detach().cpu().numpy()
        renders = []
        for vert in verts:
            if axis and angle:
                render_result = self.renderer(vert, color=[.9, .9, .8], angle=angle, axis=axis) 
            else:
    ...
    def visualize_side_result(self, vertices, verts_camed, data, reorganize_idx=None, save_img=False,hp_aes=None, centermaps=None,**kwargs): #pkps, kps, 
        """ Seperate rendering function to render the SIDE view
        """
        img_size=1024
        if reorganize_idx is not None:
        ...
        #print("images.shape in 'visualize_side_result() in visualization.py:", images.shape)

        # For the side view, we want a black background, not the original images
        images = np.zeros(images.shape)
        ...
        ### DEFINE ROTATION HERE ###
        angle = 270
        axis = [0,1,0]
        ############################

        rendered_imgs = self.visualize_renderer(verts_camed[verts_vids], images=images, reorganize_idx=new_idxs, angle=angle, axis=axis)
